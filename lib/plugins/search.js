...
anya(
	...
	async (anyaV2, pika, { args, prefix, command }) => {
		...
		if (args.join(" ").includes('spotify.com')) return pika.reply(`Url not allowed! for Urls use _${prefix + command}dl_ for urls`);
	        axios.get('https://pikabotzapi.vercel.app/search/spotify/?apikey=anya-md&query=' + encodeURIComponent(args.join(" ")))
	        .then(({data}) => {
			if (!data.status) return pika.reply(code.message || 'An Error Occured');
			let caption = '';

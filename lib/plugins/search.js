...
anya(
	{
		name: 'spotify',
		alias: ['sspotify', 'searchspotify'],
		react: '🎶',
		category: 'search',
		desc: 'Search high quality songs on Spotify',
		need: 'query',
		filename: __filename
	},
	async (anyaV2, pika, { args, prefix, command }) => {
		if (args.length < 1) return pika.reply('_Which song you want hubby?_');
		if (args.join(" ").includes('spotify.com')) return pika.reply(`Url not allowed! for Urls use _${prefix + command}dl_ for urls`);
	        axios.get(`${Config.BASE_API_URL}/search/spotify/?apikey=${Config.BASE_API_KEY}&query=${encodeURIComponent(args.join(" "))}`)
	        .then(({data}) => {
			if (!data.status) return pika.reply(data.message || 'An Error Occured');
			let caption = '';
			caption += '🏮 `Reply a number to download that song!`\n🏮 *Example :* 3\n\n';
			let count = 0;
			for (const i of data.results) {
				if (count === 24) break;
				count++;
				caption += `*[${count}] ${i.title}*\n`;
				caption += `🔗 ${i.url}\n`;
				caption += `> _type : ${i.type}_\n`;
				caption += `> _artists: ${i.artists}_\n`;
				caption += `> _duration : ${formatRuntime(i.duration)}_\n`;
				caption += `> _popularity : ${i.popularity}_\n`;
				caption += `> _*Album :*_\n`;
				caption += `> ‎   - _name : ${i.album.name}_\n`;
				caption += `> ‎   - _type : ${i.album.type}_\n`;
				caption += `> ‎   - _artist : ${i.album.artist}_\n\n\n`;
			}
			caption += `> _ID: QA49_`;
			anyaV2.sendMessage(pika.chat, {
				...
				contextInfo: {
					externalAdReply: {
						...
						title: `${Config.botname} SPOTIFY Engine`,
						body: 'Reply with a number to download song',
						thumbnailUrl: pickRandom(data.results)?.album.thumbnail,
						mediaType: 1,

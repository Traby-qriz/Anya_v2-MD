...
anya({ usage: "text", notCmd: true }, async (anyaV2, pika, { system, bot }) => {
    ...
    const handleAiFallback = async () => {
        const cai = await axios.get(`${Config.BASE_API_URL}/ai/cai_anya/?prompt=${encodeURIComponent(pika.text)}&chatid=${userId}`);
        if (cai.data?.status) return sendAiResponse(data.data?.message);
